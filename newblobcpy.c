...
int check_neighbors(int *current_position,char *FRef){
	int check_table[13];
	check_table[0] = (*current_position)-1;
	check_table[1] = (*current_position) - 161;
	check_table[2] = (*current_position) - 160;
	check_table[3] = (*current_position) -159;
	int i; //for loop variable
	int b_count = 0; //will hold sum of all the 1s
	for(i=0;i<4;i++){ //iterates through all locations to be searched
